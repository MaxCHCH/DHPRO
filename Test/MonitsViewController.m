...
- (void)viewDidLoad {
	...
	self.navigationItem.title  = @"维修监控";
...
- (void)setupSegmentedControl {
	...
	NSArray  *title_arr = @[@"全部", @"新投诉", @"已核实", @"已处理", @"已回访"];
	
	// 创建底部滚动视图
	self.mainScrollView = [[UIScrollView alloc] init];
	_mainScrollView.frame = CGRectMake(0, 20, self.view.frame.size.width, self.view.frame.size.height-20);
	...
	self.SG = [SGSegmentedControl segmentedControlWithFrame:CGRectMake(0, 20, self.view.frame.size.width, 44) delegate:self segmentedControlType:(SGSegmentedControlTypeStatic) titleArr:title_arr];
	_SG.titleColorStateSelected = [UIColor redColor];
	_SG.indicatorColor = [UIColor orangeColor];
	[self.view addSubview:_SG];
	
	UIView *linView = [[UIView alloc]init];
	linView.frame = CGRectMake(0, 43+20, [UIScreen mainScreen].bounds.size.width, 1);
