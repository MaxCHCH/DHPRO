...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        UIFont *fatFont = [UIFont systemFontOfSize:13];
...
- (void)layoutSubviews {
    ...
    self.infoLabel.frame = CGRectMake(self.bounds.origin.x + 10, self.bounds.origin.y + 15 - 7, self.infoLabel.frame.size.width, self.infoLabel.frame.size.height);
}
