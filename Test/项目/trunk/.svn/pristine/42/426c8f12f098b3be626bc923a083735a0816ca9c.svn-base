...
#define kTabBarHeight 49.0f//*[UIScreen mainScreen].bounds.size.height/480
...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr;
{
	...
	if (self != nil)
	{
		...
		_tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height - kTabBarHeight, SCREEN_WIDTH, kTabBarHeight) buttonImages:arr];
...
- (void)dealloc 
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"PostAnimate" object:nil];
...
- (void)tabBar:(LeveyTabBar *)tabBar didSelectIndex:(NSInteger)index
{
    static int num=0;
    if (index==2) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"PostAnimate" object:self userInfo:nil];
    }else if(index == 1){
        if (num>0) {
            UINavigationController *nav = (UINavigationController*)self.viewControllers[1];
            MyTeamViewController *team = nav.viewControllers[0];
            [team reload];
        }
        num++;
        [self displayViewAtIndex:index];
