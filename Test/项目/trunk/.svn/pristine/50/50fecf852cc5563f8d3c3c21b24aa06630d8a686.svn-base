...
//    [self.view addSubview:self.cancelButton = [self acquireCustomButtonWithTitle:@"<" andAction:@selector(onCancel:)]];
    [self.view addSubview:self.cancelButton = [self acquireCustomButtonWithTitle:nil imageName:@"fh-36.png" andAction:@selector(onCancel:)]];
    [self.view addSubview:self.confirmButton = [self acquireCustomButtonWithTitle:@"确认" imageName:nil andAction:@selector(onConfirm:)]];
    [self.view addSubview:self.titleLabel = [self acquireBlackTransparentOverlayLabelWithStr:@"选择封面"]];
...
- (UILabel*)acquireBlackTransparentOverlayLabelWithStr:(NSString *)title
{
    UILabel *titleLabel = [[UILabel alloc]init];
    titleLabel.userInteractionEnabled = NO;
    [titleLabel setText:title];
    [titleLabel setTextColor:[UIColor colorWithRed:0.341 green:0.388 blue:0.439 alpha:1.000]];
    titleLabel.font = MyFont(17);
//    titleLabel.layer.opacity = 0.25f;
    return titleLabel;
}
- (UIButton *)acquireCustomButtonWithTitle:(NSString*)title imageName:(NSString *)name andAction:(SEL)action
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    if (title) {
        [button setTitle:title forState:UIControlStateNormal];
        button.titleLabel.font = MyFont(15);
        [button setTitleColor:[WWTolls colorWithHexString:@"#fe5303"] forState:(UIControlStateNormal)];
    }
    else{
        button.frame = CGRectMake(0, 13, 12.5, 12.5);
...
#define kButtonViewHeight 50
    self.buttonBackgroundView.frame = CGRectMake(0, 0, CGRectGetWidth(self.view.bounds),kButtonViewHeight);//CGRectGetHeight(self.view.bounds)-kButtonViewHeight
    //底部俩按钮
#define kButtonWidth 50
    self.titleLabel.frame = CGRectMake(self.view.frame.size.width/2-30, 0, 80, kButtonViewHeight);
    self.titleLabel.font = MyFont(17);
    self.cancelButton.frame = CGRectMake(15,13 , 18, 18);//CGRectGetMinY(self.buttonBackgroundView.frame)
    self.confirmButton.frame = CGRectMake(CGRectGetWidth(self.buttonBackgroundView.frame)-kButtonWidth-15, 0, kButtonWidth, kButtonViewHeight);//CGRectGetMinY(self.buttonBackgroundView.frame)
