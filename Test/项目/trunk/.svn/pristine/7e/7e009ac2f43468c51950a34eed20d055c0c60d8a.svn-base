...
- (void)setFrame:(CGRect)frame
{
    ...
    // 1.状态标签
    _statusLabel.frame = CGRectMake(statusX-38, statusY, statusWidth, statusHeight);

    // 2.时间标签
    CGFloat lastUpdateY = statusY + statusHeight;
    _lastUpdateTimeLabel.frame = CGRectMake(statusX-38, lastUpdateY, statusWidth, statusHeight);
    
    // 3.箭头
    CGFloat arrowX = w * 0.5 - 70;
    _arrowImage.center = CGPointMake(arrowX, h * 0.5-6);
...
- (void)endRefreshing
{
    InitViewController * initView = [[InitViewController alloc]init];
    [initView checkNetworkStatus];
