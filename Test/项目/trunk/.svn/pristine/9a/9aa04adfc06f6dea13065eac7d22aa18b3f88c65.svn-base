...
@property(nonatomic,strong)NSIndexPath *lastIndexPath;//<#descript#>
@end
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        imageView.image = [UIImage imageNamed:@"xj-244"];
    }else{
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (pickerImageView.isMaskViewFlag) {
        ...
        if (minCount == 1 && self.selectAsstes.count == 1) {
            // 删除的情况下
//            NSIndexPath *i = [NSIndexPath indexPathForRow:(int)self.selectsIndexPath.lastObject inSection:indexPath.section];
            MLSelectPhotoPickerCollectionViewCell *lastcell = (MLSelectPhotoPickerCollectionViewCell *) [collectionView cellForItemAtIndexPath:self.lastIndexPath];
            MLPhotoPickerImageView *lastPickerImageView = [lastcell.contentView.subviews lastObject];
            lastPickerImageView.maskViewFlag = NO;
            
            [self.collectionViewDelegate pickerCollectionViewDidSelected:self deleteAsset:[self.selectAsstes lastObject]];
            [self.selectsIndexPath removeLastObject];
            [self.selectAsstes removeLastObject];
            [self.lastDataArray removeLastObject];
        }
        if (self.selectAsstes.count >= minCount) {
            NSString *format = [NSUSER_Defaults objectForKey:@"zdsselectphotoTip"];
            if (minCount == 0) {
                format = [NSUSER_Defaults objectForKey:@"zdsselectphotoTip"];
            }
