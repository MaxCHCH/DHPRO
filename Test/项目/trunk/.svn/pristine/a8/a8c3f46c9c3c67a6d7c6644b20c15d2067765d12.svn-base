...
-(void)initUI
{
    ...
    [DownView setBackgroundColor:[UIColor colorWithWhite:1.f alpha:0]];
...
-(void)CirculatingItmes
{
    ...
    CGFloat left = 0;
    if (_ItmesArr.count < 4) {
        left =(kW - (ButtonWidth * _ItmesArr.count))/2;
    }
    
    for (MenuLabel *Obj in _ItmesArr) {
        CGFloat buttonX,buttonY;
        buttonX = (index % HowMucHline) * ButtonWidth+left;
...
//            [buttons CancelAnimation];
    ...
    [self HidDelay:0.4f CompletionBlock:^(BOOL completion) {
        if (!weak.block) {
            return ;
        }
...
//    [self addSubview:imageView];
    self.imageView.animationImages = self.MenuData.images;
