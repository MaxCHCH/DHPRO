...
- (void)setPhotos:(NSArray *)photos
{
    ...
    _saveImageBtn.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-60, 0, btnWidth, btnWidth);
...
- (void)saveImage
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (photo.image == nil) {
            UIImageWriteToSavedPhotosAlbum(photo.placeholder, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        }else
