...
-(id)initWithOrigin:(CGPoint)origin andDiameter:(CGFloat)diameter andDelegate:(UIViewController *)controller themeColor:(UIColor *)themeColor AndImages:(NSArray*)imgs{
...
-(void)addSomeViews{
    ...
    cross.bounds = CGRectMake(0, 0, menuFrame.size.width, menuFrame.size.width);
    cross.backgroundColor = [UIColor clearColor];
    [self.mainView addSubview:cross];

    UIImageView *img = [[UIImageView alloc] init];
    img.image = [UIImage imageNamed:@"jh-100"];
    img.frame = CGRectMake(0, 0, 50, 50);
//    img.center = self.mainView.center;
    [cross addSubview:img];
...
-(void)setUpSomeDatas{
    ...
    distance = R + r + self.extraDistance + 20;
...
//        item.backgroundColor = menuColor;
        ...
        item.bounds = CGRectMake(0, 0, 52, 65);
...
-(void)menuTap:(UITapGestureRecognizer *)tapGes{
    ...
    for (NSInteger i = 0; i<menuCount; i++) {
        if ((tapGes.view.tag == i+1) && [self.menuDelegate respondsToSelector:@selector(menuDidSelected:)]) {
            [self.menuDelegate menuDidSelected:i];
        }
    }
...
- (void)setIsOpened:(BOOL)isOpened{
    _isOpened = isOpened;
    if (isOpened) {
        [self.menuDelegate menuDidOpen];
    }else [self.menuDelegate menuDidClose];
}
#pragma mark -- 点击大按钮
