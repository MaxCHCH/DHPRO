...
static CGFloat kheigh=0;

+(void)setKHeight:(CGFloat)h{
    kheigh = h;
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat pageX = (self.frame.size.width - pageWidth)/2;
        CGFloat pageY = self.frame.size.height - 24;
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(pageX, pageY, pageWidth, pageHieght)];
        _pageControl.userInteractionEnabled = NO;
        _pageControl.hidesForSinglePage = YES;
#define color(r,g,b,a) [UIColor colorWithRed:(r)/255.0 green:(g)/255.0 blue:(b)/255.0 alpha:(a)]
        _pageControl.pageIndicatorTintColor = [WWTolls colorWithHexString:@"#d8d8d8"];//白色半透明
...
#warning 加载数据在这里
        ...
        NSRange rang = [imageUrl rangeOfString:@"http://"];
        if (rang.length == 0) {
            imageView.image = [UIImage imageNamed:imageUrl];
        }else [imageView sd_setImageWithURL:[NSURL URLWithString:imageUrl]];
        //        imageView.image = [UIImage imageNamed:imageUrl];
        //tap手势
