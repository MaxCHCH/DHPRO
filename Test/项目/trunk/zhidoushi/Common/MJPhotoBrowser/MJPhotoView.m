...
@property(nonatomic,assign)BOOL hiden;//隐藏
@end
...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
            ...
            [dict setValue:_photo.url.absoluteString forKey:@"imageurl"];
            [WWRequestOperationEngine operationManagerRequest_Post:ZDS_GETBIGIMAGE parameters:dict requestOperationBlock:^(NSDictionary *dic) {
                
                NSString *bigImge = [NSString stringWithFormat:@"%@",dic[@"imageurl"]];
                _photo.url = [NSURL URLWithString:bigImge];
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        // 直接显示进度条
        if (self.photo.punchdate && self.photo.punchdate.length > 0) {
            [_photoLoadingView showLoadingWithCalendar];
        }else{
...
- (void)adjustFrame
{
    ...
    // 设置伸缩比例
    CGFloat minScale = boundsWidth / (imageWidth*2);
    if (minScale > 1) {
        minScale = 1.0;
    }
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            }else _photo.srcImageView.image = _photo.placeholder;
            //            _photo.srcImageView.image = _photo.placeholder;
            [self photoStartLoad];
        }];
    } else {
...
-(void)scrollViewDidZoom:(UIScrollView *)scrollView{
    scrollView.contentSize = CGSizeMake(scrollView.contentSize.width, scrollView.contentSize.height + _imageView.origin.y*2);
}
#pragma mark - 手势处理
...
- (void)hide
{
    ...
    [UIView animateWithDuration:duration + 0.1 animations:^{
        CGRect rect = CGRectMake(0, 0, _photo.srcImageView.bounds.size.width, _photo.srcImageView.bounds.size.height);
