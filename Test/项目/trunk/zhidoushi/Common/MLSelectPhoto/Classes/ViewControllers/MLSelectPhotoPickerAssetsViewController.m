...
#pragma mark 初始化按钮
- (void) setupButtons{
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStyleDone target:self action:@selector(back)];
    [[UINavigationBar appearance] setBarTintColor:[UIColor whiteColor]];
    [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];

    
    UIButton *leftButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [leftButton setBackgroundImage:[UIImage imageNamed:@"fh-36.png"] forState:UIControlStateNormal];
    [leftButton setFrame:CGRectMake(0, 13, 18, 18)];
    [leftButton addTarget:self action:@selector(call) forControlEvents:(UIControlEventTouchUpInside)];
    ...
    [rightButton setTitleColor:[WWTolls colorWithHexString:@"#fe5303"] forState:UIControlStateNormal];
    rightButton.titleLabel.font = MyFont(14);
    [rightButton setFrame:CGRectMake(13, 0, 50, 50)];
    [rightButton addTarget:self action:@selector(back) forControlEvents:(UIControlEventTouchUpInside)];
    
    UIBarButtonItem *rightBtn = [[UIBarButtonItem alloc] initWithCustomView:rightButton];
    [self.navigationItem setRightBarButtonItem:rightBtn];
    
    UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.view.frame.size.width/2-30, 16, 80, 20)];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.backgroundColor = [UIColor clearColor];
    [titleLabel setText:@"选择封面"];
    titleLabel.font = MyFont(17.0);
    titleLabel.textColor = [UIColor colorWithRed:0.212 green:0.267 blue:0.322 alpha:1.000];
    [self.navigationItem setTitleView:titleLabel];
    

    
//    [self.navigationController.navigationBar insertSubview:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"navbg.png"]] atIndex:1];
...
- (void)pickerCollectionViewDidCameraSelect:(MLSelectPhotoPickerCollectionView *)pickerCollectionView{
    ...
    if (self.selectAssets.count >= KPhotoShowMaxCount) {
        [self.view showMessageWithText:[NSUSER_Defaults objectForKey:@"zdsselectphotoTip"]];
        return ;
    }
...
//        if (self.minCount == 1 && self.selectAssets.count == 1) {
//            [self.assets removeLastObject];
//            [self.selectAssets removeLastObject];
//            [self.collectionView.selectsIndexPath removeLastObject];
...
//    [self showAlertMsg:@"最多选择一张图片哟^_^" yOffset:40];
