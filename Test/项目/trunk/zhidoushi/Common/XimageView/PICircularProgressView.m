...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat radius = MIN(rect.size.width, rect.size.height) / 2.0f-5;
    ...
    // Fill outerCircle with outerBackgroundColor
    UIBezierPath *inCircle = [UIBezierPath bezierPathWithArcCenter:center
                                                            radius:radius-16
                                                        startAngle:0.0
                                                          endAngle:2*M_PI
                                                         clockwise:NO];
    //    [inCircle addArcWithCenter:center
    //                           radius:radius - circleWidth
    //                       startAngle:2*M_PI
    //                         endAngle:0.0
    //                        clockwise:YES];
    ...
    if (_showText && _textColor)
    {
        ...
        {
            font = [UIFont fontWithName:@"Arial-BoldMT" size:35];
        ...
        [[WWTolls colorWithHexString:@"#535353"] setFill];
        
        [progressString drawAtPoint:origin forWidth:expectedSize.width withFont:font lineBreakMode:NSLineBreakByWordWrapping];
        origin.x = origin.x+expectedSize.width+2;
        origin.y = origin.y+5;
        [@"%" drawAtPoint:origin forWidth:expectedSize.width withFont:[UIFont fontWithName:@"Arial-BoldMT" size:12] lineBreakMode:NSLineBreakByWordWrapping];
    }
