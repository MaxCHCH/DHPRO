//
//  NARUICollectionViewFlowLayout.m
//  自定义Layout
//
...
#import "NARImageCollectionFlowLayout.h"

NSString *const WaterFallSectionHeader = @"WaterFallSectionHeader";
NSString *const WaterFallSectionFooter = @"WaterFallSectionFooter";

@interface NARImageCollectionFlowLayout ()

@property (nonatomic, weak) id <ImageFlowLayoutDelegate> delegate;
...
@implementation NARImageCollectionFlowLayout
...
{
    if (_minimum_InteritemSpacing != minimumInteritemSpacing)
    {
        _minimum_InteritemSpacing = minimumInteritemSpacing;
        [self invalidateLayout];
    }
}
...
- (void)prepareLayout
{
    ...
    self.delegate = (id <ImageFlowLayoutDelegate> )self.collectionView.delegate;
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section)
    {
        ...
        for (idx = 0; idx < itemCount; idx++)
        {
            ...
            //..调节内部成员大小与距离..//
            attributes.frame = CGRectMake(xOffset, yOffset+15, self.itemWidth, itemHeight+80);
            NSLog(@"xOffset*****%f,%f,%f,%f",xOffset,yOffset,self.itemWidth,itemHeight);
            //            [itemAttributes addObject:attributes];
            [self.allItemAttributes addObject:attributes];
            self.columnHeights[columnIndex] = @(CGRectGetMaxY(attributes.frame) + self.minimum_InteritemSpacing);
        }
