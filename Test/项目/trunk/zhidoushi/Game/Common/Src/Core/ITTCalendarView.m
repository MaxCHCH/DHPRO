...
- (void)initParameters
{
    _gridSize = CGSizeMake(29, 21);//调节按钮尺寸
...
- (GridIndex)getGridViewIndex:(ITTCalendarScrollView*)calendarScrollView touches:(NSSet*)touches
{
    ...
    ITTDINFO(@"row %ld column %ld", row, (long)column);
    index.row = row;
    index.column = column;
    return index;
}
...
- (NSString*)findMonthDescription
{
    ...
    if (!title||![title length]) {        
        title = [NSString stringWithFormat:@"%lu年%lu月", (unsigned long)[_calMonth getYear], (unsigned long)[_calMonth getMonth]];        
    }
    return title;
}
...
- (void)setSelectedAtRow:(NSUInteger)row column:(NSUInteger)column selected:(BOOL)selected
{
    NSString *key = [NSString stringWithFormat:@"%lu%lu", (unsigned long)row, (unsigned long)column];
...
- (BOOL)isGridViewSelectedEnableAtRow:(NSUInteger)row column:(NSUInteger)column
{
    ...
    if (day < 1 || day > _calMonth.days) {
        ...
        ITTDINFO(@"day is %lu", (unsigned long)day);            
    ...
    //            [_delegate calendarViewDidSelectDay:self calDay:calDay];
    //        }
    }
...
- (NSString*) getSelectedDayKey:(NSInteger)row column:(NSInteger)column
{
    ...
    return [NSString stringWithFormat:@"%04lu%02lu%02lu", (unsigned long)[cday getYear], (unsigned long)[cday getMonth], (unsigned long)[cday getDay]];
}

- (NSString*) getSelectedDayKey:(ITTCalDay *)calDay
{
    return [NSString stringWithFormat:@"%04lu%02lu%02lu", (unsigned long)[calDay getYear], (unsigned long)[calDay getMonth], (unsigned long)[calDay getDay]];
}
