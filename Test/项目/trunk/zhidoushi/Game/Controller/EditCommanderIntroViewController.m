//
//  EditCommanderIntroViewController.m
//  zhidoushi
//
...
@interface EditCommanderIntroViewController () <UITextViewDelegate>
...
@implementation EditCommanderIntroViewController

#pragma mark Life Cycle
-(void)viewWillAppear:(BOOL)animated{
    [super viewWillAppear:animated];
    //友盟打点
    [MobClick beginLogPageView:@"编辑团长介绍页面"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    //友盟打点
    [MobClick endLogPageView:@"编辑团长介绍页面"];
}    

- (void)viewDidLoad {
    [super viewDidLoad];
    //导航栏标题
    self.titleLabel.text = @"编辑团长介绍";
...
- (void)menu {
    if (self.content.text.length<1) {
        [self showAlertMsg:@"团长简介不能为空" andFrame:CGRectZero];
        return;
    }else if([WWTolls isHasSensitiveWord:self.content.text]){
        [self.view endEditing:YES];
        [self showAlertMsg:@"注意哦！团长介绍中含有敏感词！" yOffset:0];
        return;
    }
    ...
    [dictionary setObject:self.content.text forKey:@"crtorintro"];
    [dictionary setObject:self.loseway forKey:@"loseway"];
